"""Tests for LLM analyzer integration with proper mocking."""
...
def llm_config():
    ...
    return LLMConfig(
        provider="openai",
        api_key="test_key",  # This is the correct field name
...
def detection_data():
    ...
    data = DetectionData(
        ...
        metadata={}  # Initialize metadata as empty dict, not None
    )
    return data
...
class TestLLMAnalyzer:
    ...
    async def test_analyze_detection_with_openai(self, app_config, detection_data, image_paths):
        ...
        # Create analyzer with mocked session and environment
        with patch("aiohttp.ClientSession") as mock_session, \
            ...
            with patch.object(analyzer, "_crop_object_from_image", return_value=("/tmp/cropped.jpg", MagicMock())):
                ...
                # Check that OpenAI API was called (once for each object)
    ...
    async def test_analyze_detection_with_ollama(self, app_config, detection_data, image_paths):
        ...
        # Create analyzer with mocked session and file operations
        with patch("aiohttp.ClientSession") as mock_session, \
             patch("os.path.exists", return_value=True), \
             patch("builtins.open", MagicMock()), \
             patch("base64.b64encode", return_value=b"test_base64_data"):
            ...
            with patch.object(analyzer, "_crop_object_from_image", return_value=("/tmp/cropped.jpg", MagicMock())):
                ...
                # Check that Ollama API was called (once for each object)
                assert mock_session_instance.post.call_count >= 1
...
async def test_end_to_end_integration(app_config, detection_data, image_paths):
    ...
    if detection_data.metadata is None:
